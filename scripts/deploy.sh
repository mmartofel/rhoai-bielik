...
# =============================================================================
log_step "Krok 1/5: Weryfikacja wymagań wstępnych"
...
# =============================================================================
log_step "Krok 2/5: Weryfikacja tokenu HuggingFace"
...
# =============================================================================
log_step "Krok 3/5: Tworzenie namespace '${NAMESPACE}'"

# Jeśli namespace jest w stanie Terminating, poczekaj aż zniknie
NS_PHASE=$(oc get namespace "${NAMESPACE}" -o jsonpath='{.status.phase}' 2>/dev/null || echo "")
if [ "${NS_PHASE}" = "Terminating" ]; then
    log_warn "Namespace '${NAMESPACE}' jest w stanie Terminating — czekam na pełne usunięcie (max 120s)..."
    WAIT_NS=0
    ...
    if oc get namespace "${NAMESPACE}" &>/dev/null; then
        die "Namespace '${NAMESPACE}' nadal istnieje po 120s — sprawdź: oc get namespace ${NAMESPACE}"
    fi
...
# =============================================================================
# KROK 4: Secret z tokenem HuggingFace (nazwa: huggingface-token)
# =============================================================================
log_step "Krok 4/5: Tworzenie Secret 'huggingface-token' w namespace '${NAMESPACE}'"
...
# =============================================================================
# KROK 5: LLMInferenceService
# =============================================================================
log_step "Krok 5/5: Tworzenie LLMInferenceService 'bielik-11b-multinode'"
...
echo "  Pipeline Parallel: ${PIPELINE_PARALLEL_SIZE}× T4"
